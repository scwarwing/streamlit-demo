# -*- coding: utf-8 -*-
"""
Created on Sun May 16 15:42:59 2021
...
# Site setting
st.set_page_config(page_title="Mox's ML Demo", page_icon="mox_logo.png")
st.sidebar.image("regular_logo.png")
st.sidebar.title("Navigation")
tabs = ["Stock Real-time Prediction","AutoML"]
page = st.sidebar.radio("ML Pages",tabs)
...
# Stock prediction
if page == "Stock Real-time Prediction":
    start = "2015-01-01"
    today = date.today().strftime("%Y-%m-%d")
    
    st.title("Stock Real-time Prediction Demo")
    st.write('This app enables you to generate time series forecast for US stocks in real-time via Yahoo!Finance market data.')
    
    n_years = st.slider("Please select years of prediction:", 1, 5)
    period = n_years * 365
    ...
    def plot_raw_data(ticket_text):
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data['Date'], y=data['Open'], name='stock_open'))
        fig.add_trace(go.Scatter(x=data['Date'], y=data['Close'], name='stock_close'))
        fig.layout.update(title_text="Time Series Data - "+ticket_text, xaxis_rangeslider_visible=True)
        st.plotly_chart(fig)
            
    stocks = ("AAPL","GOOG","MSFT","TSLA")
    selected_stock = st.selectbox("Select default stock for prediction", stocks)
    
    user_input = st.text_input("Or type the stock ticker symbon for prediction here: " "such as AAPL")
    
    if(st.button("Predict")):
        if user_input == "":        
            data_load_state = st.text("Load data for "+selected_stock+"...")
            data = load_data(selected_stock)
            ticket_text = selected_stock
        else:
            data_load_state = st.text("Load data for "+user_input+"...")
            data = load_data(user_input)
            ticket_text = user_input
            
        data_load_state.text("Loading data for "+ticket_text+"...done")   
        
        st.subheader('Raw data - ' +ticket_text)
        st.write(data.tail())    
        ...
        st.subheader('Forecast data - ' +ticket_text)
        st.write(forecast.tail())
        
        st.write('forecast data - ' +ticket_text)
        fig1 = plot_plotly(m, forecast)
        st.plotly_chart(fig1)    
        
        st.write('forecast components - ' +ticket_text)
